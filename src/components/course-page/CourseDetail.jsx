...
import {
  changeCourseStatus,
  editCourse,
  editRequirementsAndAnnotationsCourse,
  signUpForCourse,
...
function CourseDetail({
  ...
  const [showCourseStatusEditModal, setShowCourseStatusEditModal] =
    useState(false);
  const [courseStatusEditValidated, setCourseStatusEditValidated] =
    useState(false);
  ...
  return (
    <div className="mt-2">
        ...
        <div>
          {(isAdmin || isTeacher) && (
            <Button
              variant="warning"
              className="mb-2 ms-3"
              onClick={toggleCourseEditModal}
            >
              РЕДАКТИРОВАТЬ
            </Button>
          )}
          {!isStudent && course.status == "OpenForAssigning" && (
            <Button className="mb-2 ms-3" onClick={SignUp}>
              ЗАПИСАТЬСЯ НА КУРС
            </Button>
          )}
        </div>
            ...
            <div className="d-flex align-items-stretch">
              {(isAdmin || isTeacher) && (
                <Button
                  ...
                  onClick={toggleCourseStatusEditModal}
                >
                  ИЗМЕНИТЬ
                </Button>
              )}
      ...
      <CourseEditModal
        ...
        onClose={toggleCourseEditModal}
        handleSubmit={handleCourseEditSubmit}
        validated={courseEditValidated}
        isAdmin={isAdmin}
        course={course}
      />
      <CourseStatusEditModal
        show={showCourseStatusEditModal}
        onClose={toggleCourseStatusEditModal}
        handleSubmit={handleCourseStatusEditSubmit}
        validated={courseStatusEditValidated}
        isAdmin={isAdmin}
        course={course}
      />
    </div>
