...
function CourseParticipantsTab({
  ...
  return (
    <div className="mt-5 mb-5" border="light">
      <Tabs defaultActiveKey="teachers" id="course-participants-tab" justify>
        <Tab eventKey="teachers" title="Преподаватели">
          <div className="border border-top-0 rounded-bottom-1">
            <div className="p-4">
              {(isAdmin || isMainTeacher) && (
                <div>
                  <Button className="mb-3" onClick={toggleTeacherAddModal}>
      ...
      <TeacherAddModal
        ...
        onClose={toggleTeacherAddModal}
        handleSubmit={handleTeacherAddSubmit}
        validated={teacherAddValidated}
        course={course}
      />
    </div>
  );
